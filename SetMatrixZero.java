...
public class SetMatrixZero {
	 ...
	 * No. Since we’re going to set the entire row and column to zero, 
	 * do we really need to track which cell in a row is zero? 
	 * No. We only need to know that row 2, for example, has a zero.
	 * code below implement this algorithm. We keep track in two arrays all the rows with zeros and all the columns with zeros. We then make a second pass of the matrix and set a cell to zero if its row or column is zero.
	 * */
	
	public static void setZeros(int[][] matrix) {
		int[] row = new int[matrix.length];
		int[] column = new int[matrix[0].length];
		// store the row and column index with valuse 0
