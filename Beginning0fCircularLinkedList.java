 ...
 * Output: C
 * */		
public class Beginning0fCircularLinkedList {
	/*
	 * If we move two pointers, one with speed 1 and another with speed 2, they will end up meeting if the linked list has a loop. 
	 * Why? Think about two cars driving on a track—the faster car will always pass the slower one!
	 * 
	 * The tricky part here is finding the start of the loop. 
	 * Imagine, as an analogy, two people racing around a track, one running twice as fast as the other. 
	 ...
	 * (Why? Fast Runner would have made k + 2(n - k) steps, including its head start, and Slow Runner would have made n - k steps. Both will be k steps before the start of the loop.)
	 * 
	 * Now, going back to the problem, when Fast Runner (n2) and Slow Runner (n1) are moving around our circular linked list, 
	 * n2 will have a head start on the loop when n1 enters. 
